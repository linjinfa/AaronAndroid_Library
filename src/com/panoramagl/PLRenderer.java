	...
	protected int checkHotspotsCollisionWithRay(GL10 gl, PLVector3[] ray, boolean isMoving, CGPoint screenPoint)
	{
		...
		for(int i = 0; i < elementsLength; i++)
		{
			...
			if(element instanceof PLHotspot)
			{
				...
				collisionData.points[0].setValues(vertexs[0] ,-vertexs[1]  ,-vertexs[2]);
				collisionData.points[1].setValues(vertexs[3] ,-vertexs[4]  ,-vertexs[5]);
				collisionData.points[2].setValues(vertexs[6] ,-vertexs[7]  ,-vertexs[8]);
				collisionData.points[3].setValues(vertexs[9] ,-vertexs[10] ,-vertexs[11]);
...
//				collisionData.points[3].setValues(vertexs[9] ,-vertexs[11] ,vertexs[10]);
	...
	{
		IntBuffer buffer = IntBuffer.allocate(PLConstants.kTextureMaxWidth);
		gl.glGetIntegerv(GL10.GL_MAX_TEXTURE_SIZE, buffer);
		//FileManager.writeDataToFile(buffer.get());
